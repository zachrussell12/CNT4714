    ...
    public void run() {
        ...
        if(!conveyorLocks[0] && !conveyorLocks[1]) {
            if (!conveyorSystem.isBeltTaken(assignedConveyorOne)) {
                ...
                System.out.println("\t\t\t\tStation " + workStation + " successfully acquired lock on: " + assignedConveyorOne + "\n");
        ...
        if(!conveyorLocks[0] || !conveyorLocks[1]){
            ...
            lock.lock();
            if(workLoadLeft == assignedWorkload){System.out.println("* * * * * * Routing Station " + workStation + ": * * * * CURRENTLY HARD AT WORK MOVING PACKAGES * * * * * *\n\n");}
            while (workLoadLeft != 0) {
                ...
                if(workLoadLeft == 1){
                    System.out.println("\t\t\tRouting Station " + workStation + " has " + workLoadLeft + " package group left to move.\n");
                }
                else{
                    System.out.println("\t\t\t\tRouting Station " + workStation + " has " + workLoadLeft + " package groups left to move.\n");
                }
                conveyorSystem.turnBeltOff(assignedConveyorOne);
                conveyorSystem.turnBeltOff(assignedConveyorTwo);
                System.out.println();
                if(workLoadLeft == 0){System.out.println("\n* * Station " + workStation + " Workload successfully completed. * * Station: " + workStation + " releasing/unlocking belts and going offline. * *\n");}
                break;
            }
            lock.unlock();
        }
